...
## 9. 🤖 致 AI 开发者的系统级指令 (AI System Prompt)

如果你是负责研发新主题的 AI，请**严格覆盖并执行**以下指令，必须打破默认的保守修改策略：

### 9.1 打破谨慎：大刀阔斧的设计授权 (Aggressive Redesign Authorized)
1. **停止“挤牙膏”式修改**：不要只替换基础背景色和文本色。你被完全授权对所有 CSS 变量进行**颠覆性的视觉重构**（如大面积使用 `linear-gradient`、复杂的多层 `box-shadow`、极其张扬的高对比度色彩）。
2. **拒绝平庸的纯色**：如果用户要求的是特定风格（如：赛博朋克、拟物风、毛玻璃等），请**必须**使用符合该风格的高级 CSS 特性，不要只用 `#000` 或 `#111` 敷衍。
3. **建立主次色阶 (Color Hierarchy)**：在着手修改前，先在你的 `Thought` 中想清楚该主题的 1 个主色 (Primary)、2 个辅助色 (Secondary) 以及它们对应的 Hover/Active 变体，并严格向下贯彻。

### 9.2 边缘细节防漏清单 (Edge-case Checklist)
AI 在生成主题时容易遗漏以下视觉死角，请逐一核对并重写这些变量，否则将视为研发失败：

- [ ] **悬停与交互反馈 (Hover/Active)**
  - 按钮的 `hover` 和 `active` 状态是否配置了合理的亮度变化或透明度？
  - 表格行的 `hover` 背景色（`--table-row-hover-bg-color`）是否与整体背景协调，且不会遮挡文字？
  - 导航岛的主题切换按钮的 `hover` 阴影（`--island-theme-hover-shadow`）是否生效？
  
- [ ] **“空状态”与深层文本 (Empty States & Muted Text)**
  - 首页没数据时的“空状态文本色”（`--home-empty-text`、`--home-heatmap-empty-text`）是否能看清？不要让它和背景色融为一体。
  - ECharts 图表和日历的 Tooltip（提示框）背景和边框是否已经重新适配了当前主题？（特别是暗色主题下，千万别弹出个白色的 Tooltip 刺瞎用户的眼）。
  
- [ ] **边框与分割线 (Borders & Dividers)**
  - 表格边框、首页各个卡片的边框（`--home-box-card-border`）、以及图表的坐标轴线（`--home-chart-axis-line-color`），是否正确改成了具有一点透明度的当前主题适配色？（不要用生硬的 `solid #ccc`）。

- [ ] **Element Plus 组件深度覆盖**
  - 深色模式下，是否已经彻底在 `theme.css` 底层覆盖了 `.el-button` 的所有状态？
  - 深色模式下，是否已经清洗了 `.el-table` 的原生白色背景（将相关配置设为 `transparent` 并覆盖文字色）？
